...
static int enc64(char*,byte*,int);
...
static void encode(byte*, uint*, uint);
static void decode(uint*, byte*, uint);
static MD5state* md5(byte*, uint, byte*, MD5state*);
static void sum(FILE*, char*);

char* encode_md5(byte* buf, ssize_t len)
{
	byte digest[16];
	char pr64[25];
	ssize_t i, n;
	...
	for (i=0;i<16;i++)
	{
		...
		md5hex[i*2] = hex[(digest[i]>>4) & 0xF];
		md5hex[i*2+1] = hex[(digest[i]) & 0xF];
	}
	md5hex[32] = '\0';
